...
`credit-checks.mjs` exercises admin-only credit approval, validation, idempotent replay, conflicting revisions, exclusions, immutable timestamps and complete decision history. It parses the authenticated Excel response, checking all ten admin sheets, native download/error responses, typed totals, timestamps, plain-text notes, role scope, filtered registers and history. Analysis and daily totals must reconcile without counting credit revisions twice.
