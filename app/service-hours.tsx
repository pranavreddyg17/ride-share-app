...
export function Reports({ state }: ViewProps) {
  const [filters, setFilters] = useState<ReportFilters>({}),
    [busy, setBusy] = useState(false),
    [error, setError] = useState('');
  const rides = filterRides(state, filters),
    credits = state.credits.filter((c) => rides.some((r) => r.id === c.rideId));
  return (
    <>
      <Heading
        title="Reports & exports"
        description="Excel records for school review and pilot administration."
      />
      <Filters state={state} value={filters} onChange={setFilters} />
      <section className="export-workspace">
        <div>
          <span className="caps-label">EXCEL WORKBOOK</span>
          <h2>Ride & service record</h2>
          <p>
            {rides.length} rides · {(approvedMinutes(credits) / 60).toFixed(2)}{' '}
            approved hours
          </p>
          <button
          ...
          >
            <Download />
            {busy ? 'Preparing workbook…' : 'Download .xlsx'}
          </button>
          {error && (
            <p className="error-message" role="alert">
              {error}
        ...
        <ol className="export-sheet-list">
          {[
            [
              'Driver summary',
              'Per-driver rides, waiting, driving, service time, and approved hours.',
            ],
            [
              'Ride ledger',
              'Every filtered ride with dates, exact timestamps, route, assignment and outcome.',
            ],
            [
              'Credit reviews',
              'Every credit revision with minutes, decision, reviewer, review time and reason.',
            ],
            [
              'Activity',
              'Ride events and the history of service-credit amendments.',
            ],
            [
              'Driver register',
              'Contact, vehicle, approval, and review expiry information.',
            ],
            [
              'Family register',
              'Guardian, student, consent and emergency contacts.',
            ],
            [
              'Meeting points',
              'Registered pickup and drop-off addresses and coordinates.',
            ],
          ].map(([title, detail], i) => (
            <li key={title}>
      ...
      <p className="ledger-note">
        The selected dates refer to scheduled pickups in Central Time. Registers
        include the people and meeting points in the filtered rides. The
        workbook contains personal data; use your school’s approved storage.
        Exporting does not approve service credit.
      </p>
    </>
  );
}
